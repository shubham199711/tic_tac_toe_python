...
class TieTacToe:
    ...
    def minimax(self, game, depth, current_player):
        game = list(game)
        ...
        for index in range(len(game)):
            if game[index] == 0:
                ...
                scores.append(self.minimax(tuple(game), local_depth,
                              self.get_current_opposite(current_player)))
                game[index] = 0
                moves.append(index)
    ...
    def bot_play(self):
        if self.is_game_over() == True:
            self.print_state()
            return
        best_pick = self.minimax(tuple(self.board), 0,
